<?php
namespace LiteTest\Formatter;

use LiteTest;

use Colors\Color;

class Plaintext extends TestRunner
{
	...
	public function output_results($runner)
	{

		echo PHP_EOL;

		foreach ($runner->get_results() as $case_name => $test_results) {
			$this->output_case_results($case_name, $test_results);
		}
		
		$this->print_summary();
	}
	
	protected function output_summary()
	{
		echo "\n\n";
	...
	protected function output_case_results($case_name, $test_results)
	{
		foreach ($test_results as $one_result) {
			...
			if ($one_result->passed()) {
				$this->total_results++;
				$this->output_test_pass($one_result);
			} else {
				$this->total_failed++;
				$this->output_test_fail($one_result);
			}
		}
	}

	protected function output_test_pass(TestResult $result)
	{
	...
	protected function output_test_fail(TestResult $result)
	{
		$case_name = $result->get_testcase();
		$running_time = $this->format_time($result->get_running_time());
